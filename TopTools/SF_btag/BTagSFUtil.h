...
Based on the BTagSFUtil from Michael Segala
 ...
 - When running over FastSim MC, the dataset has to be specified to retrive special correction factors:
      ...
      BTagSFUtil *BTagSF = new BTagSFUtil(BTagAlgorithm, OperatingPoint, DataPeriod, FastSimDataset)
...
class BTagSFUtil{
 ...
 public:
  ...
  BTagSFUtil(string BTagAlgorithm, TString OperatingPoint, int SystematicIndex = 0, TString FastSimDataset = "", int Seed = 123);
  ~BTagSFUtil();
