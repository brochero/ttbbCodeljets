...
void roo2Dfit(TString SystVar = ""){
  ...
  for (unsigned int ch=0; ch<3; ch++){
    ...
    RooRealVar  fit_ratio_BkgOther("fit_ratio_BkgOther", "FITTED ratio bkgOther/FullBkg", 0.4, 0.0, 1.0); 

    // Signal
    RooRealVar n_ttbb_var   ("n_ttbb_var",   "number of ttbb events",   InFile[ttbb].events[ch],   InFile[ttbb].events[ch],   InFile[ttbb].events[ch]);
    RooRealVar n_ttb_var    ("n_ttb_var",    "number of ttb events",    InFile[ttb].events[ch],    InFile[ttb].events[ch],    InFile[ttb].events[ch]);
    RooRealVar n_ttccLF_var ("n_ttccLF_var", "number of ttccLF events", InFile[ttccLF].events[ch], InFile[ttccLF].events[ch], InFile[ttccLF].events[ch]);
    RooRealVar n_ttjj_var   ("n_ttjj_var",   "number of ttjj events",   n_ttjj, n_ttjj, n_ttjj);
    RooFormulaVar kn_ttjj_var("kn_ttjj_var", "Normalized number of ttjj events", 
			      "k*n_ttjj_var", RooArgList(k, n_ttjj_var));
    
    // Background
    RooRealVar n_Bkgtt_var    ("n_Bkgtt_var",    "number of tt background events",    InFile[Bkgtt].events[ch],    InFile[Bkgtt].events[ch],    InFile[Bkgtt].events[ch]);
    RooRealVar n_BkgOther_var ("n_BkgOther_var", "number of Other background events", InFile[BkgOther].events[ch], InFile[BkgOther].events[ch], InFile[BkgOther].events[ch]);
    RooRealVar n_BkgFull_var  ("n_BkgFull_var",  "number of Total background events", InFile[BkgFull].events[ch],  InFile[BkgFull].events[ch],  InFile[BkgFull].events[ch]);

    RooFormulaVar kn_BkgFull_var  ("kn_BkgFull",  "Normalized number of background events", 
				   "k*n_BkgFull_var",  RooArgList(k, n_BkgFull_var));
    RooFormulaVar kn_Bkgtt_var    ("kn_Bkgtt",    "Normalized number of tt background events", 
				   "k*n_Bkgtt_var",    RooArgList(k, n_Bkgtt_var));
    RooFormulaVar kn_BkgOther_var ("kn_BkgOther", "Normalized number of Other background events", 
				   "k*n_BkgOther_var", RooArgList(k, n_BkgOther_var));
    

    RooArgList arg_CSV(CSV2, CSV3);
    //histograms    
    RooDataHist data_his   ("data_his",   "data Histogram",   
			    arg_CSV, InFile[data].hist2D[ch]);
    RooDataHist ttbb_his   ("ttbb_his",   "ttbb Histogram",   
			    arg_CSV, InFile[ttbb].hist2D[ch]);
    RooDataHist ttcc_his   ("ttcc_his",   "ttbb Histogram",   
			    arg_CSV, InFile[ttcc].hist2D[ch]);
    RooDataHist ttb_his    ("ttb_his",    "ttb Histogram",    
			    arg_CSV, InFile[ttb].hist2D[ch]);
    RooDataHist ttccLF_his ("ttccLF_his", "ttccLF Histogram", 
			    arg_CSV, InFile[ttccLF].hist2D[ch]);

    RooDataHist Bkgtt_his    ("Bkgtt_his",    "Bkgtt Histogram",     
			      arg_CSV, InFile[Bkgtt].hist2D[ch]);
    RooDataHist BkgOther_his ("BkgOther_his", "BkgOther Histogram",    
			      arg_CSV, InFile[BkgOther].hist2D[ch]);
    RooDataHist BkgFull_his  ("BkgFull_his",  "BkgFull Histogram",    
			      arg_CSV, InFile[BkgFull].hist2D[ch]);
       
    //pdf 
    RooHistPdf ttbb_hispdf   ("ttbb_hispdf",   "PDF for ttbb",     RooArgSet(arg_CSV), ttbb_his);
    RooHistPdf ttcc_hispdf   ("ttcc_hispdf",   "PDF for ttcc",     RooArgSet(arg_CSV), ttcc_his);
    RooHistPdf ttb_hispdf    ("ttb_hispdf",    "PDF for ttb",      RooArgSet(arg_CSV), ttb_his);
    RooHistPdf ttccLF_hispdf ("ttccLF_hispdf", "PDF for ttccLF",   RooArgSet(arg_CSV), ttccLF_his);

    RooHistPdf Bkgtt_hispdf    ("Bkgtt_hispdf",    "PDF for ttbar Bkg", RooArgSet(arg_CSV), Bkgtt_his);
    RooHistPdf BkgOther_hispdf ("BkgOther_hispdf", "PDF for Other Bkg", RooArgSet(arg_CSV), BkgOther_his);
